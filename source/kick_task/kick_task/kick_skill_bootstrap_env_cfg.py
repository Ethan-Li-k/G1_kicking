...
class BootstrapCommandsCfg:
    ...
    # Bootstrap: only sample from standing + pre-kick phases of the motion.
    kick_motion = mdp.KickMotionCommandCfg(
        ...
        # Restrict bootstrap start phase to stable stand + pre-kick.
        # Motion uses strike_phase_window=(0.35, 0.62) and pre_kick_phase_end=0.34,
        # so [0.0, 0.30] keeps us away from strike / recover frames.
...
@configclass
class BootstrapActionsCfg:
    joint_pos = mdp.JointPositionActionCfg(asset_name="robot", joint_names=[".*"], scale=0.25, use_default_offset=True)


@configclass
class BootstrapObservationsCfg:
    @configclass
...
@configclass
class BootstrapRewardsCfg:
    # Core Stage A-style kick rewards kept, but with softer scales for bootstrap.
    ...
    reward_ball_speed = RewTerm(func=mdp.reward_ball_speed, weight=8.0, params={"min_speed": 0.08})
    reward_ball_impulse = RewTerm(func=mdp.reward_ball_impulse, weight=6.0, params={"window_steps": 10})

    # Goal-direction / goal-speed completely off in bootstrap.
    ...
    # Tracking: significantly weaker than the main task; only a light motion prior.
    tracking_anchor_pos = RewTerm(func=mdp.tracking_anchor_pos_gated, weight=0.12)
    tracking_anchor_ori = RewTerm(func=mdp.tracking_anchor_ori_gated, weight=0.10)
    ...
    joint_limit = RewTerm(func=mdp.joint_pos_limits, weight=-0.03)

    # All post-kick recovery and anti-crouch / stable-stand items remain off for bootstrap.
    reward_post_kick_upright = RewTerm(func=mdp.reward_post_kick_upright, weight=0.0)
    reward_post_kick_base_height = RewTerm(func=mdp.reward_post_kick_base_height, weight=0.0)
    reward_kick_leg_retract = RewTerm(func=mdp.reward_kick_leg_retract, weight=0.0)
    reward_post_kick_recontact = RewTerm(func=mdp.reward_post_kick_recontact, weight=0.0)
    reward_post_kick_velocity_damping = RewTerm(func=mdp.reward_post_kick_velocity_damping, weight=0.0)
    reward_post_kick_joint_nominal = RewTerm(func=mdp.reward_post_kick_joint_nominal, weight=0.0)
    penalty_leg_spread = RewTerm(func=mdp.penalty_leg_spread, weight=0.0)
    penalty_post_joint_limit_stronger = RewTerm(func=mdp.penalty_post_joint_limit_stronger, weight=0.0)
    penalty_post_kick_crouch = RewTerm(func=mdp.penalty_post_kick_crouch, weight=0.0)
    reward_post_kick_stand_height = RewTerm(func=mdp.reward_post_kick_stand_height, weight=0.0)
    reward_post_kick_stable_stand = RewTerm(func=mdp.reward_post_kick_stable_stand, weight=0.0)
...
@configclass
class BootstrapTerminationsCfg:
    time_out = DoneTerm(func=mdp.time_out, time_out=True)
    # Bootstrap: slightly more forgiving height/orientation thresholds.
...
@configclass
class BootstrapEventCfg:
    # Tighter and coupled reset: ball is sampled in robot local right-front region.
    reset_base = EventTerm(
        func=mdp.reset_root_and_ball_right_front,
        mode="reset",
        params={
            ...
            # Ball relative to robot root in robot local frame:
            # +x = front, +y = left side.
...
@configclass
class BootstrapCurriculumCfg:
    # Two-stage curriculum local to the bootstrap task: A0 (stabilize) → A1 (basic kick).
    kick_skill_bootstrap = CurrTerm(func=mdp.kick_skill_bootstrap_curriculum)


@configclass
class KickSkillBootstrapEnvCfg(ManagerBasedRLEnvCfg):
    scene: SoccerBallSceneCfg = SoccerBallSceneCfg(num_envs=4096, env_spacing=3.0)

    observations: BootstrapObservationsCfg = BootstrapObservationsCfg()
    actions: BootstrapActionsCfg = BootstrapActionsCfg()
    commands: BootstrapCommandsCfg = BootstrapCommandsCfg()

    rewards: BootstrapRewardsCfg = BootstrapRewardsCfg()
    terminations: BootstrapTerminationsCfg = BootstrapTerminationsCfg()
    events: BootstrapEventCfg = BootstrapEventCfg()
    curriculum: BootstrapCurriculumCfg = BootstrapCurriculumCfg()

    def __post_init__(self):
        self.decimation = 4
        # Slightly shorter episodes are enough for bootstrap (focus on early stability + first kick).
...
@configclass
class KickSkillBootstrapPlayEnvCfg(KickSkillBootstrapEnvCfg):
    def __post_init__(self):
        super().__post_init__()
